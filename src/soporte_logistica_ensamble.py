...
class AnalisisModelosClasificacion:
    ...
    "xgboost": {
        ...
        if modelo_nombre == "logistic_regression":
            ...
            try:
                with open(f'../data/modelos/modelo_{modelo_nombre}.pkl', 'wb') as f:
...
def color_filas_por_modelo(row):
    if row["modelo"] == "decision tree":
        return ["background-color: #e6b3e0; color: black"] * (len(row) - 1) + [""]  
    
    elif row["modelo"] == "random_forest":
        return ["background-color: #c2f0c2; color: black"] * (len(row) - 1) + [""] 

    elif row["modelo"] == "gradient_boosting":
        return ["background-color: #ffd9b3; color: black"] * (len(row) - 1) + [""] 

    elif row["modelo"] == "xgboost":
        return ["background-color: #f7b3c2; color: black"] * (len(row) - 1) + [""] 

    elif row["modelo"] == "regresion lineal":
        return ["background-color: #b3d1ff; color: black"] * (len(row) - 1) + [""] 
    
    return ["color: black"] * (len(row) - 1) + [""]
